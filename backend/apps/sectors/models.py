...
class OrganizationalUnit(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["parent", "active"], name="org_unit_parent_active_idx"),
        ]
    ...
    def clean(self):
        ...
        if self.pk and self.parent_id == self.pk:
            raise ValidationError({"parent": "Uma unidade não pode ser superior a si mesma."})

        ancestor_id = self.parent_id
        visited = set()
        while ancestor_id:
            if ancestor_id == self.pk or ancestor_id in visited:
                raise ValidationError({"parent": "A hierarquia de unidades não pode conter ciclos."})
            visited.add(ancestor_id)
            ancestor_id = (
                OrganizationalUnit.objects.filter(pk=ancestor_id)
