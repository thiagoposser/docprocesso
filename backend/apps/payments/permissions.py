...
class PaymentPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        if not request.user.is_authenticated or not request.user.has_perm("payments.view_financial_data") or not request.user.has_perm("processes.view_administrativeprocess"):
            return False
        permission = "payments.view_payment" if request.method in SAFE_METHODS else (
            "payments.add_payment" if view.action == "create" else "payments.change_payment"
        )
        return request.user.has_perm(permission)

    def has_object_permission(self, request, view, obj):
        permission = "payments.view_payment" if request.method in SAFE_METHODS else "payments.change_payment"
