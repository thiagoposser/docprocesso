...
export class UserService {
  ...
  memberships(user: number) { return this.http.get<PaginatedResponse<UserSectorMembership>>('/api/user-sector-memberships/', { params: { user } }); }
  addMembership(payload: {user:number;unit:number;sector:number;function:number;active:boolean;is_primary:boolean;is_manager:boolean;starts_on:string;ends_on:string|null}) { return this.http.post<UserSectorMembership>('/api/user-sector-memberships/', payload); }
