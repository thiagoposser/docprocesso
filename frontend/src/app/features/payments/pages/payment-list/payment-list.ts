...
import { PAYMENT_STATUS_LABELS, Payment, PaymentStatus, Supplier } from '../../models/payment.models';
...
export class PaymentList {
  ...
  readonly payments = signal<Payment[]>([]); readonly suppliers = signal<Supplier[]>([]); readonly sectors = signal<FlatSectorNode[]>([]); readonly loading = signal(true); readonly error = signal(''); readonly count = signal(0); readonly next = signal<string|null>(null); readonly page = signal(1);
  readonly labels = PAYMENT_STATUS_LABELS; readonly statuses = Object.keys(PAYMENT_STATUS_LABELS) as PaymentStatus[];
  search=''; process=this.route.snapshot.queryParamMap.get('process') || ''; status=''; sector=''; supplier=''; dueFrom=''; dueTo=''; ordering='due_date';
  constructor() { this.api.suppliers().subscribe({ next: page => this.suppliers.set(page.results) }); this.sectorApi.tree('true').subscribe({ next: tree => this.sectors.set(this.flatten(tree)) }); this.load(); }
  ...
  private load(){ this.loading.set(true); this.error.set(''); this.api.list({search:this.search,process:this.process,status:this.status,sector:this.sector,supplier:this.supplier,dueFrom:this.dueFrom,dueTo:this.dueTo,ordering:this.ordering,page:this.page()}).subscribe({next:r=>{this.payments.set(r.results);this.count.set(r.count);this.next.set(r.next);this.loading.set(false);},error:e=>{this.error.set(e?.status===403?'Você não possui acesso aos dados financeiros.':'Não foi possível carregar os pagamentos.');this.loading.set(false);}}); }
