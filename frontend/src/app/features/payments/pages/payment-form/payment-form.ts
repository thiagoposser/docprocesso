...
export class PaymentForm{
  ...
  readonly processes=signal<ProcessItem[]>([]);readonly suppliers=signal<Supplier[]>([]);readonly saving=signal(false);readonly error=signal('');
  ...
  syncSector(){const process=this.processes().find(item=>item.id===Number(this.form.controls.process.value));this.form.controls.sector.setValue(String(process?.current_sector||process?.origin_sector||''));}
  submit(){this.form.markAllAsTouched();if(this.form.invalid||this.saving())return;const value=this.form.getRawValue();const payload={process:Number(value.process),sector:Number(value.sector),supplier:Number(value.supplier),description:value.description.trim(),amount:value.amount.replace(',','.'),due_date:value.due_date};this.saving.set(true);this.error.set('');const request=this.id?this.api.update(this.id,payload):this.api.create(payload);request.pipe(finalize(()=>this.saving.set(false))).subscribe({next:item=>this.router.navigate(['/pagamentos',item.id]),error:r=>this.error.set(r?.error?.detail||'Não foi possível salvar o pagamento.')});}
