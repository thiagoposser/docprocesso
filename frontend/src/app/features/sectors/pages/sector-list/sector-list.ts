...
@Component({
  ...
  template: `
    <app-page-header title="Setores" description="Gerencie os setores vinculados às unidades."><a class="btn btn-primary" routerLink="/administracao/estrutura/setores/novo">+ Novo setor</a></app-page-header>
    <section class="card border-0 shadow-sm"><div class="card-body border-bottom"><div class="row g-3"><div class="col-12 col-md-8"><label class="form-label" for="sector-search">Buscar</label><input id="sector-search" class="form-control" type="search" placeholder="Nome, código ou responsável" [(ngModel)]="search" (keyup.enter)="load()"></div><div class="col-12 col-md-4"><label class="form-label" for="sector-status">Status</label><select id="sector-status" class="form-select" [(ngModel)]="status" (change)="load()"><option value="">Todos</option><option value="true">Ativos</option><option value="false">Inativos</option></select></div></div></div>
      ...
      <div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead class="table-light"><tr><th>Setor</th><th>Unidade</th><th>Status</th><th class="text-end">Ações</th></tr></thead><tbody>
        @for (sector of sectors(); track sector.id) { <tr><td><div class="sector-name" [style.padding-left.rem]="sector.level * 1.25"><span class="tree-marker" aria-hidden="true">{{ sector.level ? '↳' : '●' }}</span><strong>{{ sector.name }}</strong>@if (sector.code) { <code>{{ sector.code }}</code> }</div></td><td>{{ sector.unit_name || 'Legado sem unidade' }}</td><td><app-status-badge [active]="sector.active" /></td><td class="text-end text-nowrap"><a class="btn btn-sm btn-outline-primary me-1" [routerLink]="['/administracao/estrutura/setores', sector.id, 'editar']">Editar</a><button class="btn btn-sm btn-outline-secondary" type="button" (click)="toggle(sector)">{{ sector.active ? 'Inativar' : 'Ativar' }}</button></td></tr> }
...
export class SectorList {
  ...
  load() { this.loading.set(true); this.error.set(''); this.api.tree(this.status).subscribe({ next: tree => { const flat = this.flatten(tree).filter(item => !this.search || `${item.name} ${item.code || ''} ${item.unit_name || ''}`.toLocaleLowerCase().includes(this.search.toLocaleLowerCase())); this.sectors.set(flat); this.loading.set(false); }, error: () => { this.error.set('Não foi possível carregar os setores.'); this.loading.set(false); } }); }
