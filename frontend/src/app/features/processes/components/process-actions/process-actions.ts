...
export class ProcessActions {
  ...
  loadActions() { this.loading.set(true); this.loadError.set(''); this.api.availableActions(this.process().id).subscribe({ next: actions => { this.actions.set(actions.filter(action => !action.integration_action)); this.loading.set(false); }, error: () => { this.loadError.set('Não foi possível carregar as ações autorizadas.'); this.loading.set(false); } }); }
