  ...
  template: `
    ...
    <section class="card border-0 shadow-sm">
      <div class="card-body border-bottom"><div class="row g-3">
        <div class="col-12 col-lg-4"><label class="form-label" for="process-search">Buscar</label><input id="process-search" class="form-control" type="search" maxlength="100" placeholder="Número, título, etapa, unidade, setor ou responsável" [(ngModel)]="search" (keyup.enter)="applyFilters()"></div>
        ...
        <div class="col-12 col-sm-6 col-lg-2"><label class="form-label" for="process-status">Situação</label><select id="process-status" class="form-select" [(ngModel)]="status" (change)="applyFilters()"><option value="">Todas</option>@for (item of statuses; track item) { <option [value]="item">{{ labels[item] }}</option> }</select></div>
        <div class="col-12 col-sm-6 col-lg-2"><label class="form-label" for="process-unit">Unidade atual</label><select id="process-unit" class="form-select" [(ngModel)]="unit" (change)="applyFilters()"><option value="">Todas</option>@for (item of units(); track item.id) { <option [value]="item.id">{{ item.name }}</option> }</select></div>
        <div class="col-12 col-sm-6 col-lg-3"><label class="form-label" for="process-sector">Setor responsável</label><select id="process-sector" class="form-select" [(ngModel)]="responsibleSector" (change)="applyFilters()"><option value="">Todos</option>@for (item of sectors(); track item.id) { <option [value]="item.id">{{ '— '.repeat(item.level) }}{{ item.name }}</option> }</select></div>
        <div class="col-12 col-sm-6 col-lg-3"><label class="form-label" for="process-stage">Etapa atual</label><select id="process-stage" class="form-select" [(ngModel)]="stage" (change)="applyFilters()"><option value="">Todas nesta página</option>@for (item of stages(); track item.id) { <option [value]="item.id">{{ item.name }}</option> }</select></div>
        <div class="col-12 col-sm-6 col-lg-2"><label class="form-label" for="process-assignee">Responsável individual</label><select id="process-assignee" class="form-select" [(ngModel)]="assignee" (change)="applyFilters()"><option value="">Todos nesta página</option>@for (item of assignees(); track item.id) { <option [value]="item.id">{{ item.name }}</option> }</select></div>
      ...
      <div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead class="table-light"><tr><th>Processo</th><th>Origem</th><th>Etapa e responsabilidade</th><th>Movimentação</th><th>Situação</th><th class="text-end">Ações</th></tr></thead><tbody>
        @for (item of processes(); track item.id) { <tr><td><strong class="d-block">{{ item.number }}</strong><span>{{ item.title }}</span><small class="d-block text-body-secondary">{{ item.process_type_name }} · criado por {{ item.created_by_name || 'Não informado' }} em {{ item.created_at | date:'shortDate' }}</small></td><td><span class="d-block">{{ item.origin_sector_name }}</span><small class="text-body-secondary">{{ item.organizational_unit_name || 'Unidade não informada' }}</small></td><td><strong class="d-block">{{ item.current_stage_name || 'Sem etapa definida' }}</strong><span>{{ item.responsible_sector_name || item.current_sector_name || 'Não definida' }}</span>@if (item.responsible_function_name || item.assignee_name) { <small class="d-block text-body-secondary">{{ item.responsible_function_name || 'Sem função' }}{{ item.assignee_name ? ' · ' + item.assignee_name : '' }}</small> }</td><td class="text-nowrap">{{ item.last_movement_at ? (item.last_movement_at | date:'short') : 'Sem movimentação' }}@if (item.last_movement_action_label) { <small class="d-block text-body-secondary">{{ item.last_movement_action_label }}</small> }<small class="d-block text-body-secondary">Atualizado {{ item.updated_at | date:'short' }}</small></td><td><span class="badge" [class]="statusClass(item.status)">{{ labels[item.status] }}</span></td><td class="text-end text-nowrap"><a class="btn btn-sm btn-light me-1" [routerLink]="['/processos', item.id]">Visualizar</a>@if (canEdit(item)) { <a class="btn btn-sm btn-outline-primary" [routerLink]="['/processos', item.id, 'editar']">Editar</a> }</td></tr> }
        @empty { @if (!loading()) { <tr><td colspan="6" class="text-center text-body-secondary py-5">Nenhum processo encontrado.</td></tr> } }
      </tbody></table></div>
...
export class ProcessList {
  ...
  units() { const values = new Map<number, string>(); for (const sector of this.sectors()) if (sector.unit && sector.unit_name) values.set(sector.unit, sector.unit_name); return [...values].map(([id, name]) => ({ id, name })).sort((a, b) => a.name.localeCompare(b.name)); }
