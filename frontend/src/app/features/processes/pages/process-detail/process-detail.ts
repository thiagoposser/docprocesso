...
@Component({
  ...
  template: `
    ...
    @if (uploadIncomplete) { <div class="alert alert-warning" role="alert"><strong>O processo foi criado, mas nem todos os arquivos foram enviados.</strong> Confira o dossiê abaixo e inclua somente os documentos que estiverem faltando.</div> }
...
export class ProcessDetail {
  readonly auth = inject(AuthService); private readonly api = inject(ProcessService); private readonly route = inject(ActivatedRoute); private readonly id = Number(this.route.snapshot.paramMap.get('id')); readonly uploadIncomplete = this.route.snapshot.queryParamMap.get('upload') === 'incomplete';
